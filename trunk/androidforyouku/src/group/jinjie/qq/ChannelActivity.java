...
public class ChannelActivity extends Activity implements View.OnTouchListener, OnScrollListener {
    private static final String TAG = "ChannelActivity";

    public static int[] mChannelButtonDefaultSize = {
            0, 0
    };

    private static final int[] CHANNEL_BUTTON_WVGA_SIZE = {
            100, 100
    };

    private static final int[] CHANNEL_BUTTON_HVGA_SIZE = {
            100, 60
    };

    private static final int[] CHANNEL_BUTTON_QVGA_SIZE = {
            70, 50
    };
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case RSS_DOWNLOAD_BEGIN:
                    ...
                    // IUrlFactory urlFactory = new
                    // YouKuCategoryUrlFactory(mCidTags.get(CustomGallery.mCurBtnPos),
                    // "1");
                    String url = urlFactory.newUrlInstance();

                    // if(rssDownloaderThread != null &&
                    // rssDownloaderThread.isAlive()) {
                    // rssDownloaderThread.interrupt();
                    // rssDownloaderThread = null;
                    // }

                    rssDownloaderThread = new RssDownloaderThread(ChannelActivity.this,
                            updateCategoryUrl(mBaseCategoryUrl, mCidTags
                                    .get(CustomGallery.mCurBtnPos), "1"), mHandler,
                ...
                case RSS_DOWNLOAD_COMPLETED:
                    ...
                    mRssContent = (Rss) msg.obj;
                    mChannelItemAdapter = new ChannelItemAdapter(ChannelActivity.this,
                            mChannelItems, mRssContent);
                    mChannelItems.setAdapter(mChannelItemAdapter);

                    // Add layout animation here
                    ...
                    //                     
                    // mChannelItems.setLayoutAnimation(controller);
                ...
                case RSS_DOWNLOAD_REFRESHED:
                    ...
                    /**
                     * Check rss contents, if the value is null assign the new
                     * value, otherwise add the new items in the previous rss
                     * contents.
                     */
                    if (mRssContent != null && !mRssContent.getChannels().isEmpty()
                            && !mRssContent.getChannels().get(0).getItems().isEmpty()) {
                        for (Item tempItem : ((Rss) msg.obj).getChannels().get(0).getItems()) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mChannelTags.clear();
        mItemTags.clear();
        ResourceUtility.getConfigTags(this, mChannelTags, mItemTags, R.array.channel_tags,
                R.array.item_tags);
        ...
        // The default downloading when you go into the channel activity at
        // first.
        rssDownloaderThread = new RssDownloaderThread(this, mBaseCategoryUrl, mHandler,
                RSS_DOWNLOAD_COMPLETED);
        rssDownloaderThread.start();
    }
    ...
    public void onScrollStateChanged(AbsListView paramAbsListView, int paramInt) {
        ...
        if (mLastItem == mRssContent.getChannels().get(0).getItems().size()
            ...
            if (mPageNumber > Integer.valueOf(mRssContent.getChannels().get(0).getValues().get(3)))
                return;
            ...
            if (!isRefreshing) {
                ...
                rssDownloaderThread = new RssDownloaderThread(this, updateCategoryUrl(
                        mBaseCategoryUrl, mCidTags.get(CustomGallery.mCurBtnPos), String
                                .valueOf(mPageNumber++)), mHandler, RSS_DOWNLOAD_REFRESHED);
                rssDownloaderThread.start();
